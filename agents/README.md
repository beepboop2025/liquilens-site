...
## Source histories and filings

Kit 1.1.0 adds `source_data.py`, a standard-library client for the separate
[research archive](https://api.seiche.info/api/v2/research-data/).
[Import its OpenAPI contract](https://api.seiche.info/api/v2/research-data/openapi.json)
or connect `https://api.seiche.info/api/v2/research-data/mcp`. The connection builder
at https://liquilens.in/start/ includes **Source data** for all supported clients.

```sh
python3 source_data.py catalog --product seiche
python3 source_data.py series --dataset ofr-fnyr --q SOFR --limit 20
python3 source_data.py observations --dataset ofr-fnyr --series FNYR-SOFR-A --start 2026-09-01 --limit 100
python3 source_data.py entities --dataset fdic-financials --q JPMorgan --limit 20
python3 source_data.py tracked-institutions --scope dossiers --q Bajaj
python3 source_data.py analysis --product undertow
```

Resolve exact series and entity identifiers before requesting observations. Use
`--entity` for an institution series. One command reads one bounded page; use its
`next_offset` explicitly for the next page. `--as-known-at` requires a timestamp
with a timezone and selects only vintages captured by that time. Pre-capture
history is not an original point-in-time record. Nulls, units, rights, source
clocks and receipts pass through unchanged; no missing value is filled with zero.
Quarterly filings, daily funding observations and chain captures have different
clocks. An archive dataset is not a calibrated risk score or execution authority.

Use `--verification` for operator tests. The client writes JSON to stdout and
never creates files, retries or fetches another page automatically. It negotiates
supported MCP versions and reads JSON or bounded SSE replies. A failed command
returns exit code 1; a source-level unavailable result returns 2.
