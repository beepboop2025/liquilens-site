...
The live [Failure Radar board](https://api.liquilens.in/api/failure-radar/board) contains **0 red, 1 orange, 3 yellow and 15 green** rows among institutions with a sufficiently recent vetted dossier. ESAF Small Finance Bank is not isolated because a dramatic adjective was chosen. It was selected for this article because its published evidence creates the strongest current tension across tier, movement, regulatory distance, funding structure and market repricing.
...
[Read Seiche](https://seiche.info/articles/) for the system question: is dollar-funding capacity tightening? Stay with [LiquiLens](https://liquilens.in/articles/) for the institution question: which balance sheet carries the exposure and what evidence is missing? Then [read Undertow](https://liquilens-undertow.com/articles/) for the execution question: can risk be transferred without moving the market? The sequence is a diagnostic funnel, not three votes on the same claim.

For the deeper conversion path, the [six-week LiquiLens proof pilot](https://liquilens.in/pilot/) tests these public screening rules against a controlled counterparty book. The public article remains fully readable whether or not the reader takes that step.

## Sources, method, and limits

- [Failure Radar board](https://api.liquilens.in/api/failure-radar/board), the source of the institution row, clocks, tiers and missing lenses.
- [Historical validation record](https://api.liquilens.in/api/failure-radar/validation), including PCA, funding and hazard diagnostics with misses preserved.
- [Three-market evidence index](https://api.liquilens.in/api/evidence/markets), which states the construction-PIT status and eligibility boundary for India, the US and Europe.
- [US NDFI watch](https://api.liquilens.in/api/us-radar/ndfi), used only as a cross-market institution context.
- [Seiche live overview](https://api.seiche.info/api/overview) and [Undertow public board](https://api.seiche.info/undertow/board.json), displayed as system and market context, never institution-score inputs.
