...
2. Open **Fetch bank evidence**. The example JSON uses `cosmos-ucb`, Cosmos Co-operative Bank. Change `slug` to another covered institution if needed. The `banking_specialisation_coverage` tool at the same MCP endpoint lists valid slugs; coverage is not a census.
...
4. Select **Execute workflow** once, then open **Keep complete evidence** and select **JSON** in the output panel to read the nested research record.
...
The revised layout was visually inspected in the native **n8n 2.37.11** editor on **2026-09-08**, using Chrome 152 at **1440 × 900** and **1280 × 800** in both light and dark themes. Three numbered notes explain the three steps, with every line visible and each node centered beneath its note. Rendered bounds checks confirmed that text stayed inside the notes and notes stayed clear of the nodes. The revision changes presentation and node labels while preserving the execution graph, MCP parameters and output-checking JavaScript.

Two manual editor executions on **n8n 2.37.11 with Node 24.19.0** also passed verification. A temporary copy forwarded a live bank review with synthetic traffic labels; its final output matched the upstream structured evidence exactly, including all 28 top-level fields. A second copy received a deliberately injected MCP tool error and stopped at **Fetch bank evidence**, without executing **Keep complete evidence**. These temporary endpoints and execution results are not included in the downloadable workflow. All seven recipe contract tests passed.
